...
export const eventAnalytics = asyncHandler(async (req: Request, res: Response) => {
  const data = await analyticsService.eventAnalytics(req.params.id, req.user!.id);
  res.json({ success: true, data });
});

export const exportOrders = asyncHandler(async (req: Request, res: Response) => {
  const csv = await analyticsService.ordersCsv(req.params.id, req.user!.id);
...
export const exportAttendees = asyncHandler(async (req: Request, res: Response) => {
  const csv = await analyticsService.attendeesCsv(req.params.id, req.user!.id);
  res.setHeader('Content-Type', 'text/csv; charset=utf-8');
